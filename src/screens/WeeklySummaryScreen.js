...
import { groupByWeek, getWeeklyStats, getSentimentColor, getSentimentBgColor, sentimentEmoji } from '../utils/sentimentUtils';
...
export default function WeeklySummaryScreen() {
  ...
  const weeklyArray = Object.entries(weeklyGroups).sort((a, b) => new Date(b[0]) - new Date(a[0]));
  ...
  return (
    <ScrollView style={styles.scrollView}>
      {weeklyArray.length === 0 ? (
        ...
        weeklyArray.map(([weekStart, weekEntries]) => {
          const stats = getWeeklyStats(weekEntries);
          const weekEnd = new Date(weekStart);
          weekEnd.setDate(weekEnd.getDate() + 6);
          const weekRange = `${weekStart} - ${weekEnd.toLocaleDateString('tr-TR')}`;
          
          return (
            <View key={weekStart} style={styles.summaryCard}>
              <Text style={styles.dateTitle}>Hafta: {weekRange}</Text>
                ...
                renderItem={({ item }) => (
                  <View style={[styles.entry, { borderLeftColor: getSentimentColor(item.ai?.sentiment), backgroundColor: getSentimentBgColor(item.ai?.sentiment) }]}>
                    ...
                    <View style={styles.entryMeta}>
                      ...
                      <Text style={styles.timeText}>{new Date(item.when).toLocaleDateString('tr-TR')} {new Date(item.when).toLocaleTimeString('tr-TR', { hour: '2-digit', minute: '2-digit' })}</Text>
                    </View>
                  </View>
                )}
              />
